...
async function seed(db, label) {
  console.log(`--- SEEDING DB: ${label} ---`);
  try {
    const products = await getDocs(collection(db, 'products'));
    for (const d of products.docs) { await deleteDoc(doc(db, 'products', d.id)); }
    const categories = await getDocs(collection(db, 'categories'));
    for (const d of categories.docs) { await deleteDoc(doc(db, 'categories', d.id)); }
...
async function run() {
  ...
  await seed(dbNamed, 'NAMED');
  await seed(dbDefault, 'DEFAULT');
}
