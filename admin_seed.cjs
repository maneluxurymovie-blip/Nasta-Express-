...
// Wait, I should try to specify the database ID if it's not default
const namedDb = admin.firestore(config.firestoreDatabaseId);
...
async function run() {
  const dbs = [namedDb]; // Try only the named one first
  
  for (const targetDb of dbs) {
    console.log(`--- SEEDING DB: ${targetDb._databaseId || 'default'} ---`);
    try {
