...
async function run() {
  ...
  console.log(`Found ${products.size} existing products. Deleting...`);
  for (const d of products.docs) {
    await deleteDoc(doc(db, 'products', d.id));
  }

  const categories = await getDocs(collection(db, 'categories'));
  console.log(`Found ${categories.size} existing categories. Deleting...`);
  for (const d of categories.docs) {
    await deleteDoc(doc(db, 'categories', d.id));
  }

  const categoryMap = {};
  const catNames = ['Snacks', 'Sweets'];
  ...
  // Final verification
  const finalProducts = await getDocs(collection(db, 'products'));
  console.log(`VERIFICATION: Products in DB: ${finalProducts.size}`);
  
  const finalCats = await getDocs(collection(db, 'categories'));
  console.log('VERIFICATION: Categories in DB:');
  finalCats.forEach(d => console.log(`- ${d.data().name} (${d.id})`));
}
